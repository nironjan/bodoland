...
const TopPostCard = ({ title, coverImageUrl, views, likes, maxViews }) => {
  ...
  const safeMaxViews = Number(maxViews) || 1;
  const viewPercentage = ((safeViews / safeMaxViews) * 100).toFixed(0);
  ...
  return (
    <div className="bg-white py-4 flex flex-col gap-3 border-b border-gray-100">
      ...
      <div className="relative w-full h-1.5 bg-gray-100/60 rounded-full overflow-hidden mt-1">
        <div
